...
from .load_data import cnf_parse_pyg


class NeuroSATDataset(InMemoryDataset):
    r"""
    ...
    def processed_dir(self):
        name = "sr{}to{}_p{}".format(self.args.min_n, self.args.max_n, self.args.n_pairs)
    ...
    def process(self):
        ...
        for n_var in range(self.min_n, self.max_n+1):
            ...
            for problems_idx in range(lower_bound, upper_bound):
                ...
                # TODO: check is it the pair setting or throw away one of them
                ...
                graph = cnf_parse_pyg(iclauses, n_vars, n_clauses, n_nodes)
